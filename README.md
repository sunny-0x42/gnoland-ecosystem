...
Open http://127.0.0.1:3310. `/admin` signs in an account that already exists in `data/auth.json`. The site does not create new admin accounts. On Vercel, the same JSON is read from the `ADMIN_AUTH` environment variable when that file is missing.
